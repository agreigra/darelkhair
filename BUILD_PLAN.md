...
## Feature 13 - appartment review ✅ DONE

> Built guest apartment reviews + admin moderation. **Schema**: new `Review` model (rating 1–5, optional comment, `@@unique([apartmentId, userId])` so one editable review per guest per apartment; cascades on apartment/user delete) + relations; migration `feature_13_reviews`. **Backend** `modules/reviews` (imports `BookingsModule`): public `GET /apartments/:id/reviews` (paginated list + aggregate summary — average, count, per-star distribution via `aggregate`+`groupBy`); guest `GET /apartments/:id/reviews/me` (own review + `canReview`), `POST` (upsert, **gated on a CONFIRMED stay** via new `BookingsService.hasConfirmedStay`), `DELETE /me`; admin `GET /admin/reviews` + `DELETE /admin/reviews/:id` (moderation). Author identity is privacy-safe ("First L.", never surname/email) on the public side; audit logs on upsert/delete. **Frontend** `features/reviews`: a reusable `StarRating` (display + interactive picker), `ReviewSummary` (big average + distribution bars), `ReviewList`, and a `ReviewForm` (upsert/delete own) composed into `ApartmentReviews`, embedded on the apartment detail page (form shown only to eligible signed-in guests); admin `/admin/reviews` moderation `DataTable` (localized apartment title, stars, author, delete-with-confirm). New `reviews` + `admin.reviews` i18n + nav key (FR/AR/EN); admin dropdown link. Verified: migrate + typecheck + build both apps; live smoke — eligible guest upsert (avg 5→4, count stays 1 on edit), public summary/distribution, ineligible→403, rating=6→400, anon→401, admin list + non-admin→403 + admin delete→200 with summary recomputed to 0.
